...
# ── matugen (config + the templates it renders from) ──
# These were versioned but never linked, so a fresh box cloned them into the
# repo and then ran matugen against templates that were not in ~/.config.
